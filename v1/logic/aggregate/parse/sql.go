			...
			if column.Type.Options != nil {
				...
				if len(mod.SearchFieldKey(mod_field.FieldName)[KeyTypePrimary]) == 0 &&
					...
					if column.Type.Options.Default != nil {
						logger.Warn(context.Background(), "table %s field %s key %v ignore primary key default value", mod.TableName, mod_field.FieldName, mod.SearchFieldKey(mod_field.FieldName)[KeyTypePrimary][0])
					}
				}
