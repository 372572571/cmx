...
type Message struct {
	...
	Name                string
	UpName              string
	MessageType         message_model.MessageReferenceType // message type
	Fields              []message_model.MessageField       // message field
...
func NewMessage(df config.Definition, fn ...Option) *Message {
	msg := &Message{
		Fields:              make([]message_model.MessageField, 0),
...
func (m *Message) InitNode(reference string) {
	...
	if m.Fields, found = config.GetDefaultConfig().GetDefinition().GetMessageField(reference); !found {
		logger.Fatalf(context.Background(),"not found message %s", reference)
	}
	m.Reference = reference
	m.Name = router.Field
	m.UpName = util.ToCamelCasing(m.Name)
	m.MessageType = message_model.RefTypeMessage
	for _, v := range m.Fields {
		if v.Ref.Type == message_model.RefTypeMessage {
			child := NewMessage(
				m.Definition,
...
func (m *Message) WriterMessage(rw *strings.Builder) {
	if m.MessageType != message_model.RefTypeMessage {
	...
	for _, v := range m.Fields {
		...
		// rw.WriteString(fmt.Sprintf("/* %s */\n", v.ColumnName))
		switch v.Ref.Type {
		case message_model.RefTypeField:
			rw.WriteString(m.writerTypeField(v, idx))
		case message_model.RefTypeEmpty:
		...
		default:
			logger.Fatalf(context.Background(), "not support type %s", v.Ref.Type)
		}
	}
	rw.WriteString("}")
	rw.WriteString("\n")
...
// writer type message
func (m *Message) writerTypeMessage(v message_model.MessageField, idx int) string {
	rw := strings.Builder{}
	...
	if child, found := m.child[v.ColumnName]; found {
		...
		if v.Comment != "" {
			commentTag = fmt.Sprintf("%s %s", strconv.Quote(v.Comment), m.Definition.GetEnumComment(v.OneOf))
		}
		...
		logger.Fatalf(context.Background(), "not found child %s", v.ColumnName)
	}
	return rw.String()
}

// writer type self
func (m Message) writerTypeSelf(v message_model.MessageField, idx int) string {
	rw := strings.Builder{}
...
// writer type field
func (m Message) writerTypeField(v message_model.MessageField, idx int) string {
	rw := strings.Builder{}
...
func (m Message) writerTypeMessageField(v message_model.MessageField, idx int) string {
	rw := strings.Builder{}
	if len(v.Ref.Select) != 1 {
		logger.Fatalf(context.Background(), "not support select %v", v.Ref.Select)
	}
	slv := v.Ref.Select[0]
	if fields, found := m.Definition.GetMessageField(v.Ref.Ref); found {
		tf := message_model.MessageField{}
		...
		if slv != tf.ColumnName {
			logger.Fatalf(context.Background(), "not found table field %s", slv)
		}
		tfComment := v.Comment
		if tfComment == "" {
			tfComment = tf.Comment
		}

		commentTag := fmt.Sprintf("%s %s",
			strconv.Quote(tfComment),
		...
		rw.WriteString(rf)
		rw.WriteString("[(grpc.gateway.protoc_gen_openapiv2.options.openapiv2_field)={")
		rw.WriteString(fmt.Sprintf(`title: %s`, strconv.Quote(tfComment)))
		rw.WriteString(" ")
		description := m.Definition.GetEnumComment(tf.OneOf)
		if description != "" {
			rw.WriteString(fmt.Sprintf(`description: %s`, m.Definition.GetEnumComment(tf.OneOf)))
		...
		logger.Fatalf(context.Background(), "not found table field %s", v.Ref.Ref)
	}
	return rw.String()
}

func (m Message) parserValidator(validator string, foo message_model.FieldOneOf) (result string, found bool) {
...
func (m Message) Tag(col, validator string, foo message_model.FieldOneOf, comment, serializer string) string {
	rw := strings.Builder{}
	// if comment != "" {
	...
	if validator, found := m.parserValidator(validator, foo); found {
		...
		logger.Fatalf(context.Background(), "build gotags error %s tag %s", col, validator)
	}
	return rw.String()
}
