...
func Generated(referenceString string) (b []byte, err error) {
	...
	if !found {
		...
		return []byte{}, fmt.Errorf("not found %s", referenceString)
	}
...
func getOneofDefinition(m *parse.Model) string {
	...
	if !ok {
		out, err := yaml.Marshal(tp.Definition)
		if err != nil {
			logger.Fatalf(context.Background(), "marshal error %v", err)
		} else {
			logger.Fatalf(context.Background(), "not found %s", string(out))
		}
	}
	result := lo.Filter(dm, func(i message_model.Field, idx int) bool {
		return i.OneOf.Ref != ""
	})
	slices.SortFunc(result, func(i message_model.Field, j message_model.Field) int {
...
func getModelToProto(repoName, modelName string, m *parse.Model) string {
	...
	for _, mf := range messageField {
		if mf.Inhibit == string(message_model.Inhibit) {
			continue // skip field
		}
			...
			case "time.Time":
				...
				default:
					logger.Fatalf(context.Background(),
						"not support model: %s field: %s time to %s",
